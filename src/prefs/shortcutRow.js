    ...
    class ShortcutRow extends Adw.ActionRow {
        constructor(settings, gettext) {
            ...
            const shortcutLabel = new Gtk.ShortcutLabel({
                ...
                accelerator: this._settings.get_strv('activate-one-thing-extended')[0],
                valign: Gtk.Align.CENTER,
                halign: Gtk.Align.CENTER,
            });

            this._settings.connect('changed::activate-one-thing-extended', () => {
                shortcutLabel.set_accelerator(
                    this._settings.get_strv('activate-one-thing-extended')[0]
                );
            });
            ...
            this.connect('activated', () => {
                ...
                controller.connect('key-pressed', (_widget, keyval, keycode, state) => {
                    ...
                    const globalShortcut = getAcceleratorName(
                        keyval,
                        keycode,
                        mask,
                        'activate-one-thing-extended'
                    );

                    if (globalShortcut === null)
                        return Gdk.EVENT_STOP;

                    this._settings.set_strv('activate-one-thing-extended', [globalShortcut]);
                    editor.destroy();
                    return Gdk.EVENT_STOP;
                });
